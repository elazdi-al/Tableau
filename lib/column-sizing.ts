...
export function calculateColumnWidths(
  ...
  const { mode, containerWidth, minColumnWidth, maxColumnWidth, showRowNumbers } = config
  
  // Account for row numbers column
  const rowNumbersWidth = showRowNumbers ? 48 : 0
  const availableWidth = containerWidth - rowNumbersWidth
  const totalColumns = columnsData.length

  if (mode === 'fixed') {
    // Use predefined column widths
    return columnsData.reduce((acc, { column }) => {
      acc[column.id] = Math.max(minColumnWidth, Math.min(maxColumnWidth, column.width))
      return acc
  ...
  if (mode === 'auto') {
    ...
    const widths = columnsData.map(({ column, cellValues }) => {
      const headerWidth = estimateTextWidth(column.name, { weight: 'medium', size: 14 })
      ...
      return {
        id: column.id,
        naturalWidth: Math.max(minColumnWidth, Math.min(maxColumnWidth, maxContentWidth + padding))
      }
    })

    const totalNaturalWidth = widths.reduce((sum, { naturalWidth }) => sum + naturalWidth, 0)

    if (totalNaturalWidth <= availableWidth) {
      // All columns fit naturally
      return widths.reduce((acc, { id, naturalWidth }) => {
        acc[id] = naturalWidth
        return acc
      }, {} as Record<string, number>)
    } else {
  ...
  if (mode === 'balanced') {
    ...
    const widths = columnsData.map(({ column, cellValues }) => {
      const headerWidth = estimateTextWidth(column.name, { weight: 'medium', size: 14 })
      const avgContentWidth = cellValues.length > 0 
        ? cellValues.reduce((sum: number, value: unknown) => 
            sum + estimateTextWidth(String(value || ''), { weight: 'normal', size: 14 }), 0
          ) / cellValues.length
        : 50
      ...
      return {
        id: column.id,
        width: Math.max(minColumnWidth, Math.min(maxColumnWidth, balancedWidth))
      }
    })
    ...
    if (totalWidth > availableWidth) {
      ...
      return widths.reduce((acc, { id, width }) => {
